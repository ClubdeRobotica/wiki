...
Con esas premisas se decidió por no utilizar microcontrolador para la Etapa de Control, puesto que a pesar de que en la actualidad estos no son costosos, para utilizarlos es necesario una infraestructura extra(por ejemplo: soporte, programador, PC). El control de manera analógica puede ser fiable, pero a baja velocidad.
...
Las de salidas de ambos comparadores se conecta a las entradas de la '''Etapa de Control'''
